...
Public Class level2
    ...
    Dim lebar As Integer = 165
    Dim bagi5 As Integer = lebar / 5

    Dim gerakan As Integer = 10

    Dim penentucek2 As Integer = 0

    Dim win As Integer
    Dim winner As Integer = 68
    ...
    Private Sub jalan_Tick(sender As Object, e As EventArgs) Handles jalan.Tick
        ...
        If y = yawal And x > a And x < a + 124 Then
            movey = True
            pantulanbarsound()
            ...
            ElseIf a + (bagi5 * 4) <= x And x <= a + (bagi5 * 5) Then
                ...
                movex = True
            End If
        ElseIf y < yawal + 150 And movey = False Then
            y += yplus
            If y > yawal + 100 And a + 124 < x Then
                gameover()
                Me.Close()
                MessageBox.Show("You Lost!", "Sorry :(", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ElseIf y > yawal + 100 And x < a Then
                gameover()
                Me.Close()
                MessageBox.Show("You Lost!", "Sorry :(", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
    ...
    Private Sub level2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For i = 0 To jumlahobstacle
            ...
            For j = 0 To jumlahobstacle
                lokasia += 50
                If (i = 2) Or (j = 1) Or (i = jumlahobstacle - 1) Or (j = jumlahobstacle - 1) Or (i = 3) Or (j = 2) Or (i = jumlahobstacle - 2) Or (j = jumlahobstacle - 2) Then
                    randomwarna = 0
